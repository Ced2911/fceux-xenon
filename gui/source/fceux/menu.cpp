...
static int FindGameSaveNum(char * savefile, int device) {
    printf("savefile => %s\r\n", savefile);
    int n = -1;
    int romlen = strlen(emu.getRomName());
    int savelen = strlen(savefile);

    int diff = savelen - romlen;

    if (strncmp(savefile, emu.getRomName(), romlen) != 0)
        return -1;
...
static int MenuInGame() {
    ...
    GuiText titleTxt(emu.getRomName(), 22, ColorGrey);
...
static int MenuGameSaves(int action) {
    ...
    std::string title = emu.getEmuName();
    title.append(" - In Game menu");
    GuiText titleTxt(title.c_str(), 28, ColorGrey);
    titleTxt.SetAlignment(ALIGN_LEFT, ALIGN_TOP);
    titleTxt.SetPosition(50, 50);
    ...
    sprintf(browser.dir, "%s%s", "uda:/", emu.getFolderName());
    BrowseDevice(emu.getFolderName(), "uda:/");
    ...
    for (i = 0; i < browser.numEntries; i++) {
        ...
        if (n >= 0) {
            ...
            snprintf(filepath, 1024, "uda:/%s/%s", emu.getFolderName(), saves.filename[j]);
        ...
        if (ret > -3) {
            ...
            if (action == 0) // load
            {
                ...
                switch (saves.type[ret]) {
                    case FILE_SRAM:
                        emu.LoadSram(filepath);
                        menu = MENU_EMULATION;
                        break;
                    case FILE_SNAPSHOT:
                        emu.LoadState(filepath);
                        menu = MENU_EMULATION;
                        break;
                }
            } else // save
            {
                if (ret == -2) // new SRAM
                {
                    ...
                    if (i < 100) {
                        MakeFilePath(filepath, FILE_SRAM, emu.getRomName(), i);
                        emu.SaveSram(filepath);
                        menu = MENU_GAME_SAVE;
                ...
                } else if (ret == -1) // new Snapshot
                {
                    ...
                    if (i < 100) {
                        MakeFilePath(filepath, FILE_SNAPSHOT, emu.getRomName(), i);
                        menu = MENU_GAME_SAVE;
                        emu.SaveState(filepath);
                    }
...
static int MenuBrowseDevice() {
    ...
    sprintf(title, "%s - Load Game", emu.getEmuName());
...
static int MenuOptions() {
    ...
    while (menu == MENU_NONE) {
        ...
        if (ret >= 0 || firstRun) {
            firstRun = false;
            //
            if(options.v[ret].curr>=options.v[ret].max)
                options.v[ret].curr=0;
            
            emu.GetOption(&options);
...
static int MainMenu() {
    ...
    GuiText titleTxt(emu.getEmuName(), 28, ColorGrey);
    titleTxt.SetAlignment(ALIGN_LEFT, ALIGN_TOP);
    titleTxt.SetPosition(50, 50);
